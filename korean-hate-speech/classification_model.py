...
from transformers import (
    ...
    XLMRobertaTokenizerFast, XLNetConfig, XLNetTokenizerFast,PreTrainedTokenizerFast,
)
...
import sys
sys.path.append('/root/team26/khaiii')
...
class ClassificationModel:
    def __init__(
        ...
        if num_labels:
            ...
            # self.config = config_class.from_pretrained(model_name, num_labels=num_labels, position_embedding_type="relative_key", **self.args.config)
        ...
        if model_name in [
        ...
        elif model_type == "xlmroberta2":
            # tokenizer1 = Tokenizer(models.Unigram())
            # tokenizer1.normalizer = normalizers.Sequence(
            #     [
            #         normalizers.Replace("``", '"'),
            #         normalizers.Replace("''", '"'),
            #         normalizers.NFKD(),
            ...
            #     ]
            # )
            # tokenizer1.pre_tokenizer = pre_tokenizers.Metaspace()
            # special_tokens = ["<cls>", "<sep>", "<unk>", "<pad>", "<mask>", "<s>", "</s>"]
            # trainer = trainers.UnigramTrainer(
            #     vocab_size=25000, special_tokens=special_tokens, unk_token="<unk>"
            # )
            # tokenizer1.model = models.Unigram()
            # tokenizer1.train(["/root/team26/korean-hate-speech/unlabeled/unlabeled_comments.txt"], trainer=trainer)
            # cls_token_id = tokenizer1.token_to_id("<cls>")
            # sep_token_id = tokenizer1.token_to_id("<sep>")
            # tokenizer1.post_processor = processors.TemplateProcessing(
            #     single="$A:0 <sep>:0 <cls>:2",
            #     pair="$A:0 <sep>:0 $B:1 <sep>:1 <cls>:2",
            #     special_tokens=[("<sep>", sep_token_id), ("<cls>", cls_token_id)],
            # )
            # tokenizer1.decoder = decoders.Metaspace()
            # self.tokenizer = tokenizer1
            self.tokenizer = XLMRobertaTokenizer.from_pretrained("/root/team26/DeepOffense/examples/korean/my_xlmr", do_lower_case=self.args.do_lower_case, **kwargs)
    ...
    def load_and_cache_examples(
        ...
        if os.path.exists(cached_features_file) and (
            ...
            features = convert_examples_to_features(
                ...
                sep_token_extra=bool(args.model_type in ["roberta", "camembert", "xlmroberta","xlmrobertaxl", "longformer"]),
                ...
                stride=args.stride,
                add_prefix_space=bool(args.model_type in ["roberta", "camembert", "xlmroberta","xlmrobertaxl", "longformer"]),
