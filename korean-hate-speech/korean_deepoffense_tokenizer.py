...
from korean_deepoffense_tokenizer_config import LANGUAGE_FINETUNE, TEMP_DIRECTORY, SUBMISSION_FOLDER, \
    MODEL_TYPE, MODEL_NAME, language_modeling_args, args, SEED, RESULT_FILE, GOOGLE_DRIVE, DRIVE_FILE_ID
...
# predictions, raw_outputs = model.predict(eval_df)
# eng_test_preds = predictions.tolist()


# with open("eng_pretrained.csv", 'w') as f:
#     writer = csv.writer(f)
#     writer.writerows(eng_test_preds)
# print("COMPLETED PRETRAIN")


# Change directory name containing English pretrained model
# MODEL_NAME = os.path.join(MODEL_NAME, "best_model")
...
if args["evaluate_during_training"]:
    for i in range(args["n_fold"]):
        ...
        model = ClassificationModel("xlmroberta_korean_tokenizer", MODEL_NAME, num_labels=3, args=args,
                                    use_cuda=torch.cuda.is_available())  # You can set class weights by using the optional weight argument
        train_df, eval_df = train_test_split(train, test_size=0.1, random_state=SEED * i)

        # model.train_model(train_df,multi_label=True, eval_df=eval_df, macro_f1=macro_f1, weighted_f1=weighted_f1, accuracy=sklearn.metrics.accuracy_score)
        # model.tokenizer = KhaiiiApi();
        model.train_model(train_df, eval_df=eval_df, macro_f1=macro_f1, weighted_f1=weighted_f1, accuracy=sklearn.metrics.accuracy_score)
        model = ClassificationModel("xlmroberta_korean_tokenizer", args["best_model_dir"], num_labels=3, args=args,
                                    use_cuda=torch.cuda.is_available())
