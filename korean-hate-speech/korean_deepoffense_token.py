...
from korean_deepoffense_config_2 import LANGUAGE_FINETUNE, TEMP_DIRECTORY, SUBMISSION_FOLDER, \
...
from common.print_stat import print_information_multi_class
...
if args["evaluate_during_training"]:
    for i in range(args["n_fold"]):
        ...
        model = ClassificationModel("xlmroberta2", MODEL_NAME, num_labels=3, args=args,
                                    use_cuda=torch.cuda.is_available()) 
        train_df, eval_df = train_test_split(train, test_size=0.1, random_state=SEED * i)
        model.train_model(train_df, eval_df=eval_df, macro_f1=macro_f1, weighted_f1=weighted_f1, accuracy=sklearn.metrics.accuracy_score)
        model = ClassificationModel("xlmroberta2", args["best_model_dir"], num_labels=3, args=args,
                                    use_cuda=torch.cuda.is_available())
